...
# Description
#############
# IntronPointer for finding intron Retention using RNA-Seq Data:
# require four matrices as input: 
# y=raw counts of reads corresponding to different terms
# (exons, junction and introns) X samples
# RMmeber= realtion of terms (exons, junction and introns) X introns
# where skipping junction marked via 3 instead of 1
...
iPrnaseqFunction <- function(counts=y,RMM= RMmeber,contrastM=contrastM,designM=designM,Groups=Groups){
#Main function#
...
#Running limma by contrast
	...
	SumOfPvalues <- t(as.matrix(pvalue1tail)) %*% ( (1 * (RMMix > 0)) + ((-1)*(auxMatrix ==0.5))) + colSums(1*(auxMatrix ==0.5))
	NumberOfPvalues <- colSums((auxMatrix + RMMix) !=0)
	EqPvalues<- as.matrix(unlist(lapply(1:length(SumOfPvalues), function(x) sumPvalsMethod(SumOfPvalues[x],NumberOfPvalues[x]))))
	constraints <- (colSums((RMMix > 0)* 1) >0) &  (colSums((auxMatrix>0)*1) >0) # there should be intron and atleast skipping Junctions
	...
	tGE<- as.matrix(t(as.matrix(t)) %*% ((RMMix+auxMatrix) > 0)*1) # now it sums up the t-statistics of term that are used for summing up pvalue
